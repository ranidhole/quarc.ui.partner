angular.module('uiGenApp')
  .controller('InterviewsListController', function InterviewsListController(QCONFIG, QuarcService, Restangular, $stateParams, $filter, moment,currentJob ) {
    const Page = QuarcService.Page;

    const vm = this;
    vm.job = currentJob || {};
    vm.buckets = QCONFIG.APPLICANT_STATES;
    Page.setTitle(`${vm.job.role?vm.job.role+ " - ":''}${$stateParams.status?$stateParams.status:''} Interviews`); // set page title
